...
const CreateUpdateDialog = forwardRef<HTMLElement, CreateUpdateDialogProps>(
  ({ datec, refetch, className }: CreateUpdateDialogProps, ref) => {
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent className="max-w-[425px] p-7 md:max-w-[650px]">
          <DialogHeader className="text-left">
            <DialogTitle>{isEditing ? 'Editar' : 'Criar'} DATEC</DialogTitle>
            <DialogDescription>
              Preencha as informações abaixo para {isEditing ? 'editar' : 'criar'} um
              {!isEditing && ' novo'} DATEC.
            </DialogDescription>
          </DialogHeader>
          <CreateUpdateForm datec={datec} onClose={handleClose} />
        </DialogContent>
      </Dialog>
    )
  },
