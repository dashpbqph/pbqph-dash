...
const DeleteDialog = forwardRef<HTMLElement, DeleteDialogProps>(
  ({ guideline, refetch, className }: DeleteDialogProps, ref) => {
    const [open, setOpen] = useState(false)
    const { mutateAsync: deleteGuideline, isPending } = api.guideline.delete.useMutation({
      onSuccess: () => {
        refetch()
        setOpen(false)
      },
    ...
    return (
      <AlertDialog open={open} onOpenChange={setOpen}>
        ...
        <AlertDialogContent className="space-y-2 bg-white">
          <AlertDialogHeader>
            <AlertDialogTitle>
              Deseja realmente remover a Diretriz {guideline.name}?
            </AlertDialogTitle>
            <AlertDialogDescription className="text-xl-1">
              Esta ação não pode ser desfeita. Isso vai remover permanentemente a diretriz e todos
          ...
          <AlertDialogFooter className="mt-0">
            ...
            <DialogDeleteButton
              isLoading={isPending}
              deleteFn={() => deleteGuideline({ id: guideline.id })}
            />
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
