...
export default function CreateUpdateForm({ ita, onClose }: CreateUpdateDialogProps) {
  const isEditing = !!ita

  const createForm = useForm<z.infer<typeof itaCreateFormSchema>>({
    resolver: zodResolver(itaCreateFormSchema),
    defaultValues: {
      name: ita?.name || '',
    },
  })
  ...
  return (
    <Form {...createForm}>
      <form className="flex flex-col gap-3" onSubmit={createForm.handleSubmit(handleSubmit)}>
        ...
        />
        <DialogFooter>
          <DialogSubmitButton isUpdating={isEditing} isLoading={isSubmiting} subject="ITA" />
        </DialogFooter>
      </form>
    </Form>
  )
