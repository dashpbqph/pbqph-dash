...
const DeleteDialog = forwardRef<HTMLElement, DeleteDialogProps>(
  ({ egt, refetch, className }: DeleteDialogProps, ref) => {
    const [open, setOpen] = useState(false)
    const { mutateAsync: deleteEGT, isPending } = api.egt.delete.useMutation({
      onSuccess: () => {
        refetch()
        setOpen(false)
      },
    ...
    return (
      <AlertDialog open={open} onOpenChange={setOpen}>
        ...
        <AlertDialogContent className="space-y-2 bg-white">
          <AlertDialogHeader>
            <AlertDialogTitle>Deseja realmente remover a entidade {egt.name}?</AlertDialogTitle>
          ...
          <AlertDialogFooter className="mt-0">
            ...
            <DialogDeleteButton isLoading={isPending} deleteFn={() => deleteEGT({ id: egt.id })} />
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    )
